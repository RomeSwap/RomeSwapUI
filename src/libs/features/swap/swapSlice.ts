...
export const fetchSPLAddress = createAsyncThunk(
    ...
    async ({solAddress, selType}:{solAddress: string, selType: "input"|"output"}) => {
        try{
...
export const swapSlice = createSlice({
    ...
    reducers: {
        ...
        setToken: (state, action: PayloadAction<{token: Token, type: 'input'|'output'}>) => {
        ...
        swapInputOutput: (state) => {
            [state.inputToken, state.outputToken] = [state.outputToken, state.inputToken]
        },
        setUserbalance: (state, action: PayloadAction<{amount: number, type: "input"|"output"}>) => {
            console.log(`userBal: ${action.payload.amount} decimals: ${state.inputToken.decimals}`)

            switch (action.payload.type) {
                case 'input':
                    state.inputToken.userBalance = Number(action.payload.amount / 10 ** state.inputToken.decimals)
                    return
                case 'output':
                    state.outputToken.userBalance = Number(action.payload.amount / 10 ** state.outputToken.decimals)
                    return
            }
        },
        ...
        setEvmAddress: (state, action: PayloadAction<{address: Address, type: "input"|"output"}>) => {
            switch (action.payload.type) {
                case 'input':
